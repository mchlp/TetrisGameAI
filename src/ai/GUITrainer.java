...
/**
 * Trains an AI {@link Population} and displays the gameplay graphically. Extends the {@link GUIBrain} class, which is
 * used to determines the best move and send it to the {@link GameController}. Each organism in the population plays one
 * game during each generation before the population is bred.
 */
public class GUITrainer extends GUIBrain {
     ...
     * @param gameController The {@link GameController} linked to the game.
    ...
    /**
     * If an organism is training, updates the training time of the population. Otherwise, updates the statistics of the
     * organism and advances to the next organism. If all organisms have been tested, {@link #mPopulation} is evolved
    ...
    public void update(double deltaTime) {
        ...
        switch (mGameProcessor.getmGameState()) {
            case PLAYING:
                // if the current organism is still playing a game, add the time passed to the total training time
