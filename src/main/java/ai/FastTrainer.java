...
/**
 * Trains an AI {@link Population} without display the gameplay graphically,
 * which allows the training to take place at a much faster rate. Extends the
 * {@link Brain} class, which is used to determines the best move. Each organism
 * in the population plays {@value #NUM_GAMES_PER_ORGANISM} games during each
 * generation before the population is bred.
 */
public class FastTrainer extends Brain {
	...
	/**
	 * <code>true</code> if training is currently taking place, <code>false</code>
	 * if paused.
	 */
	private boolean mTraining;

	/**
	 * The time when the last move in the game was made in milliseconds. Used to
	 * keep track of the amount of time the population has been training for.
	 */
	...
	/**
	 * The highest number of moves made by an organism during one game during this
	 * training session.
	 */
	private int mMaxNumMoves;

	/**
	 * The game number that the organism currently being trained is on. When it
	 * reaches {@value #NUM_GAMES_PER_ORGANISM}, it is reset back to zero for the
	 * next organism.
	 */
	...
	/**
	 * Updates the time when the last move was made in the game, which is used to
	 * update the amount of time that population has been training for.
	 */
	private void updateTrainTime() {
		long currentTimeMillis = System.currentTimeMillis();
		// add the time since the last move was made to the total time the population
	...
	/**
	 * Starts a game to test the {@link #mCurOrganism}. If the {@link #mCurOrganism}
	 * has played {@value NUM_GAMES_PER_ORGANISM} games, the next organism in
	 * {@link #mPopulation} is assigned to {@link #mCurOrganism}. If all organisms
	 * in {@link #mPopulation} have been tested, {@link #mPopulation} is evolved and
	 * testing restarts from the first organism.
	 *
	 * @param deltaTime
	...
	public void update(double deltaTime) {
		if (mTraining) {
			...
			if (mGameNum < NUM_GAMES_PER_ORGANISM) {
				// if the current organism has not finished all of its games, start a game for
				// that organism
				startGame();
				mGameNum++;
			} else {
				...
				if (mCurOrganismIndex == mPopulation.getNumOrganisms() - 1) {
					// if all the organisms in the population have been tested, evolve the
					// population and restart
					try {
						mPopulation.evolve();
					} catch (IOException e) {
	...
	private void startGame() {
		...
		while (mGameProcessor.getmGameState() == GameState.PLAYING) {
			...
			ArrayList<ControllerKeys> bestMoves = getBestMove(mGameProcessor.getmGrid(),
					mGameProcessor.getmCurTetromino());
			for (ControllerKeys move : bestMoves) {
				// for each move in the optimal set of moves, call its corresponding function in
				// the GameProcessor
				switch (move) {
				case LEFT:
		...
		if (moves > mMaxNumMoves) {
			// if the number of moves made this game is higher than the maximum number of
			// moves
			mMaxNumMoves = moves;
		}
