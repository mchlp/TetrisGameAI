...
/**
 * An abstract class that acts wrapper around the {@link Brain} which sends
 * moves calculated by the {@link Brain} to the {@link GameController}.
 */
public abstract class GUIBrain extends Brain {

	/**
	 * The score of the game when the last move was implemented. This is used to
	 * figure out when the last move sent to the {@link #mGameController} has been
	 * applied and a new Tetromino has appeared.
	 */
	...
	/**
	 * If the game is not over and a new Tetromino has appeared, get the next set of
	 * optimal moves and send it to the {@link #mGameController}. If the game is
	 * over, update the statistics of the {@link #mCurOrganism} with the current
	 * game.
	 *
	 * @param deltaTime
	...
	public void update(double deltaTime) {
		...
		case PLAYING:
			if (mPrevScore != mGameProcessor.getmScore()) {
				...
				// if the last move has been applied, meaning a new tetromino has spawned, get a
				// new set of optimal moves
				ArrayList<ControllerKeys> moves = getBestMove(mGameProcessor.getmGrid(),
