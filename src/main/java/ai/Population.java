...
/**
 * Represents a population in the genetic algorithm training process. Among
 * other values, it stores an array of {@link Organism}s, a list of elite
 * {@link Organism}s, statistics of the population, and the {@link File} to save
 * the population. It implements the {@link Serializable} interface to allow to
 * it to be exported to a file and loaded from a file for later use.
 */
public class Population implements Serializable {
	...
	/**
	 * The percentage of the population that will be reproducing to produce the next
	...
	/**
	 * Stores the elite {@link Organism}s. Every generation, the fittest
	 * {@link Organism} is added to this list. If there are more than
	 * {@value MAX_NUMBER_OF_ELITES} organisms, the organism with the lowest fitness
	 * is removed. During every breeding session, each of the organisms in this list
	 * are added to the pool of reproducing organisms.
	 */
	private ArrayList<Organism> mElites;

	/**
	 * The file to save the population to. This is marked <code>transient</code> as
	 * it is not saved when the population is serialized and saved to a file.
	 */
	private transient File mSaveFile;

	/**
	 * The generation of organisms the population on. Equivalent to the number of
	 * times the population has been bred plus one.
	 */
	...
	/**
	 * The fitness of the fittest organism in the {@link #mElites} list. This is the
	 * fittest organism that has ever bred in the population.
	 */
	 ...
	 *
	 * @param saveFile
	 *            The file to save the population.
	 */
	 ...
	 *
	 * @param loadFile
	 *            The file containing the population.
	 * @return The population stored in the file.
	 * @throws IOException
	 *             An error has occurred when reading the file.
	 * @throws ClassNotFoundException
	 *             An {@link Population} object cannot be found in the file.
	 */
	...
	/**
	 * Evolves the population through selection and breeding of the population. It
	 * advances the population to the next generation. The population is written to
	 * the {@link #mSaveFile}.
	 *
	 * @throws IOException
	 *             An error has occurred when writing to the file.
	 */
	...
	/**
	 * Selects the fittest organisms from the current generation and adds them to a
	 * group of organisms that will be used in the breeding stage. Each organism in
	 * the {@link #mElites} list is added to this group as well.
	 *
	 * @return The group of organisms that will be reproducing. It will contain
	 *         <code>{@value
	...
	public Organism[] selectAndKill() {
		...
		// copies the fittest organisms in the current generation to the reproducing
		// group while leaving just enough space for the elites
		Organism[] survivors = Arrays.copyOfRange(mOrganisms, 0,
				(int) (mOrganisms.length * REPRODUCING_PERCENTAGE) - mElites.size());
	...
	/**
	 * Generates a new generation of organisms by randomly selecting two organisms
	 * in the parents group and breeding them until {@value #NUMBER_OF_ORGANISMS}
	 * organisms have been produced.
	 *
	 * @param parents
	 *            The group of parent organisms from the last generation.
	 */
	private void breed(Organism[] parents) {
		int childIndex = 0;
		while (childIndex < NUMBER_OF_ORGANISMS) {
			// randomly select two organisms in the parent group, breeds them, and adds the
			// child produced to the population
	...
	/**
	 * Adds an organism into {@link #mElites} list. If the capacity of the list is
	 * exceeded, the organism with the lowest fitness is removed.
	 *
	 * @param elite
	 *            The organism to add to the elite list.
	 */
	...
	/**
	 * @return The fittest organism in the {@link #mElites} list. If the list is
	 *         empty, the first organism in the population. If the first organism
	 *         has not played a single game, <code>null</code>.
	 */
