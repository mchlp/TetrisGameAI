...
/**
 * An abstract class that acts as the "brain" of an AI {@link Organism}. Uses
 * the genes in the {@link Genome} of the {@link Organism} to calculate the next
 * move to make in the game. Classes that extend this abstract class will need
 * to assign an organism to use ({@link #mCurOrganism}) for calculating the next
 * move and to relay the moves calculated by this class to the actual game.
 */
public abstract class Brain implements Updatable {
	...
	/**
	 * Percentage of Tetrominos that are left to slowly descend to the bottom rather
	 * than dropped immediately when {@link #mFastMode} is disabled.
	 */
	private static final double DROP_RATE_FOR_SLOW_MODE = 0.3;

	/**
	 * The {@link Organism} that will be used to make decisions as to which move to
	 * make.
	 */
	protected Organism mCurOrganism;

	/**
	 * The {@link GameProcessor} that is linked to the game that the AI is currently
	 * playing.
	 */
	protected GameProcessor mGameProcessor;

	/**
	 * If enabled, the AI will immediately drop all tetrominos after they are in
	 * their desired horizontal position and orientation. If disabled, the AI will
	 * only drop {@value #DROP_RATE_FOR_SLOW_MODE} of tetrominos (mimics a human
	 * playing).
	 */
	...
	/**
	 * Calculates the optimal set of moves to make using the {@link Genome} of
	 * {@link #mCurOrganism} when provided with a {@link GameGrid} and the current
	 * {@link Tetromino}.
	 *
	 * @param grid
	 *            Represents the current state of the game grid.
	 * @param curTetromino
	 *            The current tetromino that is dropping.
	 * @return An ArrayList that contains the optimal set of moves to make
	 *         represented as {@link ControllerKeys}.
	 */
	protected ArrayList<ControllerKeys> getBestMove(GameGrid grid, Tetromino curTetromino) {
		...
		// loop through all possible rotations (maximum of 4 possible rotation states
		// for each tetromino)
		for (int numRotations = 0; numRotations < 4; numRotations++) {

			// loop through all possible horizontal translations (from left edge to right
			// edge)
			for (int numTranslate = -(grid.getmWidth() / 2); numTranslate <= grid.getmWidth() / 2; numTranslate++) {

				// create a clone of the current game state and tetromino to apply the current
				// set of testing moves on
	...
	/**
	 * Calculates the rating for a game state described by a {@link GameGrid} and a
	 * {@link Tetromino} using a {@link Genome}.
	 *
	 * @param grid
	 *            Represents the current state of the game grid.
	 * @param genome
	 *            The genome to use to calculate the rating.
	 * @param tetromino
	 *            The current tetromino, which has already dropped to its final
	 *            resting position.
	...
	protected double getRating(GameGrid grid, Genome genome, Tetromino tetromino) {
		...
		// for each gene, multiply the gene value by the corresponding value in the game
		// state and add it to the current rating
	 ...
	 * @param grid
	 *            Represents the current state of the game.
	 * @return The number of filled cells in the grid that are touching the left or
	 *         right edge.
	 */
	 ...
	 * @param grid
	 *            Represents the current state of the game.
	 * @return The sum of the number of blocks above each hole (an empty cell that
	 *         has a filled cell above it) in the grid.
	 */
		...
		for (int i = 0; i < grid.getmWidth(); i++) {
			...
			for (int j = 0; j < grid.getmHeight(); j++) {
				if (grid.isFilled(i, j)) {
					...
					int height = grid.getmHeight() - j;
					if (!(i == 0)) {
						// if not the first column, add the absolute difference in column height to the
						// sum
